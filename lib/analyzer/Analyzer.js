...
(function()
{
  ...
  {
    var sum = 0, result = probability.map(function(){ return []; });

    probability = probability.sort().reverse().map(function(val)
    {
      sum += val;
      return sum;
    });
  ...
  Analyzer.generateStates = function(particles, bases)
  {
    ...
    return particles.map(function(particleSet, basisIndex)
    {
      return new global.State(particleSet, bases[basisIndex]);
    });
  };
